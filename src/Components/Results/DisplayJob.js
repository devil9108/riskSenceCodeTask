  ...
  return (
    <>
      <Row key={props.data.id} justify={"space-between"}>
        ...
        <Col>
          <Title level={4} style={{ textAlign: "right" }}>
            &#x20b9; {props.data.salarymin} - {props.data.salarymax} / hr
          </Title>
        </Col>
      </Row>
      <Row>
        <Building style={{ width: "15px" }} />
        <Text style={{ color: "#8eb6d8", fontWeight: 600, margin: "0px 10px" }}>
          Epic Coders
        </Text>
        <EnvironmentOutlined
          style={{
      ...
      <Row style={{ margin: "20px 0px" }}>
        <Paragraph>{props.data.desciption}</Paragraph>
      </Row>
      <Row>
        <Col>
          {requiredSkills.map((skill, index) => (
            <Tag style={{ borderRadius: "16px" }} key={index}>
              <Text style={{ fontSize: "16px" }}>{skill}</Text>
            </Tag>
          ))}
        </Col>
      </Row>
